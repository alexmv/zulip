    ...
    *,
    ...
    def idempotency_decorator(
        ...
        def wrapper(*args: ParamT.args, **kwargs: ParamT.kwargs) -> WorkReturnT:
            ...
            with transaction.atomic(durable=durable, savepoint=savepoint):
                try:
                    # Here, we select the matching row from
                    # idempotency table and apply a row-lock on it
                    # during the whole transaction, but abort
                    # (nowait=True) if another concurrent transaction
                    ...
                    try:
                        ...
                        # probable cause for this exception. This is for debugging clarity
...
def idempotent_endpoint(
    ...
    def _wrapped_view_func(
        ...
        **kwargs: ParamT.kwargs,
        ...
        """
        The first step in applying idempotency. Decorates a
        non-idempotent view (e.g. send_message_backend).

        We insert the idempotency row which initially represents a new
        unattempted work.  In the end and after calling the view (and
